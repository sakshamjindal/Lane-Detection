...
class Atrous_ResNet_features(nn.Module):
    ...
    def __init__(self, block, layers,num_classes, pretrained=False,weight_locs=None):
        ...
        if weight_locs is not None:
            print('Loading pretrained classifier for initialization')
            resnet = models.resnet101(False)
            resnet.fc = nn.Linear(512*4,num_classes)
            load_weights = torch.load(weight_locs)
            resnet.load_state_dict(load_weights["model_weights"])
            self.conv1 = resnet.conv1
            self.bn1 = resnet.bn1
            self.layer1 = resnet.layer1
            self.layer2 = resnet.layer2
...
class DeepLabv3_plus(nn.Module):
    def __init__(self, num_classes, weight_locs,pretrained):
        super(DeepLabv3_plus, self).__init__()
        block = Atrous_Bottleneck
        self.resnet_features = Atrous_ResNet_features(block, [3, 4, 23],num_classes + 1, pretrained,weight_locs)
